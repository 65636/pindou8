...
export function BeadStudio() {
  ...
  const dimensions = patternDimensions(source, width);
  const changePatternSize = (value: number) => {
    setWidth(value);
    if (value < 53) setSmallRegionSize(0);
    else if (value > 52) setSmallRegionSize(DEFAULTS.smallRegionSize);
  };
      ...
      <section className="studio-layout">
          <aside className="panel controls-panel">
            ...
            <div className="preset-row">
              {[52, 78, 104].map((value, index) => <button key={value} className={width === value ? "active" : ""} onClick={() => { changePatternSize(value); setMaxColors([8, 10, 12][index]); }}>{["小", "中", "大"][index]} <b>{value}</b></button>)}
            </div>

            <RangeField label="图纸大小" value={width} min={16} max={256} unit="豆" onChange={changePatternSize} />
            ...
            <p className="dimension-note">预计尺寸：{dimensions.width} × {dimensions.height} 豆</p>
          </aside>
            ...
            <div className="canvas-column">
              ...
              <div className="canvas-toolbar">
                <div className="tool-group" aria-label="编辑工具">
                  {([['view', '查看'], ['brush', '画笔'], ['eraser', '橡皮'], ['picker', '取色'], ['fill', '填充']] as [Tool, string][]).map(([value, label]) => <button key={value} className={tool === value ? "active" : ""} onClick={() => setTool(value)} disabled={!pattern}>{label}</button>)}
                  <button className={replaceOpen ? "active" : ""} onClick={toggleColorReplace} disabled={!pattern}>换色</button>
                </div>
              ...
              {replaceOpen && pattern && (
                <section className="replace-popover" aria-label="批量换色面板">
              ...
              <div className={`canvas-stage ${pattern ? "has-pattern" : ""}`}>
                {pattern ? (
                  <canvas ref={canvasRef} onPointerDown={handleCanvasPointer} aria-label="可编辑拼豆图纸" />
                ) : (
            ...
            {pattern ? (
              <>
                ...
                <div className="material-list">
                  {stats.colors.map((item) => (
                    <button key={item.index} className={selectedColor === item.index ? "selected" : ""} onClick={() => { setSelectedColor(item.index); setTool("brush"); }}>
